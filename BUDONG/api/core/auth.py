...
async def authenticate_user(email: str, password: str, db: Session) -> Optional[TUser]:
    """사용자 인증 (OAuth2 표준)"""
    user = db.query(TUser).filter(TUser.email == email).first()
    if not user:
        return None
